...
std::string JsonEmitter::emitPreamble() {
    ...
    code += "            if (!v.has_value()) { j = nullptr; return; }\n";
    code += "            if (v.type() == typeid(asn1::runtime::BitString)) {\n";
    ...
    code += "            } else if (v.type() == typeid(int64_t)) {\n";
    code += "                j = std::any_cast<int64_t>(v);\n";
    code += "            } else if (v.type() == typeid(int)) {\n";
    code += "                j = std::any_cast<int>(v);\n";
    code += "            } else if (v.type() == typeid(bool)) {\n";
    code += "                j = std::any_cast<bool>(v);\n";
    code += "            } else if (v.type() == typeid(std::string)) {\n";
    code += "                j = std::any_cast<std::string>(v);\n";
    code += "            } else if (v.type() == typeid(double)) {\n";
    code += "                j = std::any_cast<double>(v);\n";
    code += "            } else {\n";
    code += "                j = nullptr;\n";
    code += "            }\n";
    code += "        }\n";
    code += "    static void from_json(const json& j, std::any& v) {\n";
    code += "        if (j.is_object() && j.contains(\"hex\") && j.contains(\"bit_length\")) {\n";
    code += "            asn1::runtime::BitString bs;\n";
    code += "            bs.bit_length = j.at(\"bit_length\").get<size_t>();\n";
    code += "            std::string hex = j.at(\"hex\").get<std::string>();\n";
    code += "            for (size_t i = 0; i + 1 < hex.size(); i += 2)\n";
    code += "                bs.data.push_back(static_cast<uint8_t>(std::stoul(hex.substr(i, 2), nullptr, 16)));\n";
    code += "            v = std::move(bs);\n";
    code += "        }\n";
    code += "    }\n";
    code += "};\n";
